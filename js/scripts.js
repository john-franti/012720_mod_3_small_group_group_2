...
function makeDetails(astronaut) {
  ...
  const confirmButton = makeConfirmButton(astronaut);
...
function makeConfirmButton(astronaut) {
  const newButton = document.createElement("button");
  if (astronaut.confirmed) {
    newButton.className = "btn btn-success";
    newButton.textContent = "Grooming Confirmed";
  } else {
    newButton.className = "btn btn-info";
    newButton.textContent = "Confirm Appointment";
    newButton.addEventListener("click", function() {
      fetch(`${APPOINTMENTS_URL}/${astronaut.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json"
        ...
        .then(json => {
          if (json.id === astronaut.id) {
            newButton.parentNode.replaceChild(makeConfirmButton(json), newButton);
          }
        });
    });
  }
